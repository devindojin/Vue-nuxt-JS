        ...
        <div class="container">
          ...
          <div class="blog-cats mt-4 pt-2 pt-md-3 mt-lg-5 pt-lg-0">
            <nuxt-link
              ...
              :class="[
                cat.toLowerCase().replace(/ /g, '-') == $route.params.slug
                  ? 'selected'
                  : '',
              ]"
            >
          ...
          <div class="blog-list mt-4 pt-md-2 mt-lg-4 pt-lg-3">
            <BlogListItem
              v-for="(post, index) of loadedPosts"
              :post="post"
              :key="index"
            />
          </div>
          ...
          >
            <div
              class="text-center"
              v-if="currentPage * blogsPerPage < loadedPosts.length"
            >
              <DotsLoader />
            </div>
          </div>
...
export default {
  ...
  created() {
    ...
    this.loadedPosts.push(...this.$config[0].blog.blogs)
    if (this.$route.params.slug && this.$route.params.slug != '') {
      //console.log(this.loadedPosts);
      //this.tagName = this.$context.tagName;
      this.loadedPosts = this.loadedPosts.filter(this.filterByCategory)
  ...
  methods: {
    ...
    filterByCategory(posts) {
      var cats_arr = []
      for (var i = 0; i < posts.categories.length; i++) {
        cats_arr.push(
          posts.categories[i].categories_id.name
            .toLowerCase()
            .replace(/ /g, '-')
        )
      }
      //console.log(cats_arr);
      if (cats_arr.indexOf(this.$route.params.slug) === -1) {
        cats_arr = []
        return false
      } else {
        return true
  ...
  computed: {
    ...
    visibleBlogs() {
      ...
      return this.loadedPosts.slice(0, blogToShow)
    },
  },
  head() {
    if (this.$config[0].blog.blogPage) {
      return {
        title: this.$config[0].blog.blogPage.seo.seo_title +" - "+ this.$route.params.slug,
